...
@ApplyOptions<Command.Options>({
  name: '미리보기채널변경',
  description: '(미리보기 전용)머핀봇의 미리보기 채널을 변경해요.',
  ...
  public async messageRun(msg: Message) {
    ...
    if (this.container.channel === 'RELEASE')
      return await msg.reply(
        '해당 봇은 정식 버전 전용이라 미리보기 채널을 변경할 수 없어요.',
      )

    const user = await this.container.database.preview_channel.findFirst({
      where: {
      ...
      embeds: [
        {
          ...
          description: `현재 ${msg.author.username}님이 속한 채널: ${user.channel.toLowerCase()}`,
          fields: [
            {
              name: `${EXPERIMENTAL} 채널`,
              value:
                '해당 채널은 다음 머핀봇의 메이저 버전을 개발하거나, 실험적인 기능을 테스트하는 채널이에요. 따라서 매우 불안정할 수 있어요.',
              inline: false,
            },
            {
              name: `${DEV} 채널`,
              value:
                `해당 채널은 머핀봇의 마이너 버전을 주로 테스트 하거나,${EXPERIMENTAL} 채널에서 어느정도 안정이 된 버전을 받는 채널이에요.\n` +
          ...
          components: [
            {
              ...
              options: [
                ...
                {
                  label: '취소',
                  description: `채널을 ${user.channel}로 유지해요.`,
